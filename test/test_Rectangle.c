...
void test_Rectangle_init(void) {
    Rectangle * rectangle = x_allocInit(Rectangle, (Point){ 0, 1 }, 2, 3);
    TEST_ASSERT_EQUAL_UINT32(2, Rectangle_getWidth(rectangle));
    TEST_ASSERT_EQUAL_UINT32(3, Rectangle_getHeight(rectangle));
    TEST_ASSERT_EQUAL_UINT8(2 * 3, Shape_area(x_castTo(Shape, rectangle)));
    x_dealloc(rectangle);
}
void test_Rectangle_setters(void) {
    ...
    TEST_ASSERT_EQUAL_UINT8(4 * 5, Shape_area(x_castTo(Shape, rectangle)));
    x_dealloc(rectangle);
}
void test_Rectangle_makeSquare(void) {
    ...
    TEST_ASSERT_EQUAL_UINT8(4 * 4, Shape_area(x_castTo(Shape, rectangle)));
    x_dealloc(rectangle);
}